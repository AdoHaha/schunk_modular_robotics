        ...
        do
        {
            ...
            for ( double sign = -1.0; sign <= 1.0; sign += 2.0 ) // generate an alternating sign: -1.0 / +1.0
            {
                ...
                while ( !position_reached )
                {
                    ...
                    std::cout << "  Actual angle: " << std::setw(7) << hand.GetAxisActualAngle( axis_index ) << " deg";
                    std::cout << ",  actual velocity: " << std::setw(7) << hand.GetAxisActualVelocity(axis_index) << " deg/s";
                    std::cout << ",  reference velocity: " << std::setw(7) << hand.GetAxisReferenceVelocity(axis_index) << " deg/s\n";
                    backupangles=hand.GetAxisActualAngle(hand.all_axes);
                    if ( sign > 0.0 )
